...
const normalizeItems = async (body) => {
  ...
  for (const item of body.items) {
    ...
    if (!menuItem) {
      throw new Error(
        `Menu item "${item.foodItem}" was not found`
      );
    }
    ...
    const sizeCategory =
      item.sizeCategory.trim().toLowerCase();
...
const addOrder = async (req, res) => {
  try {
    ...
    // Optional customer description/note
    const description =
...
const updateOrder = async (req, res) => {
  try {
    ...
    // Preserve existing description unless
    // a new description is explicitly provided.
    let description =
      order.description || '';
